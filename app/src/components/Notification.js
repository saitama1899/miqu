const Notification = ({ message, feedback = 'good' }) => {
  ...
  const tipo = feedback === 'good' ? 'successAlert' : 'errorAlert'
  const exclamacion = feedback === 'good' ? 'Bien!' : 'Ups!'

  return (
    <div className={'flex p-4 text-sm mt-2 ' + tipo} role='alert'>
      ...
      <div>
        <span className='font-medium'>{exclamacion}</span> { message }
      </div>
    </div>
  )
}
